    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.testimonis.update", [$testimoni->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $testimoni->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('judul') ? 'is-invalid' : '' }}" type="text" name="judul" id="judul" value="{{ old('judul', $testimoni->judul) }}">
                @if($errors->has('judul'))
                    <div class="invalid-feedback">
                        {{ $errors->first('judul') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('testimoni') ? 'is-invalid' : '' }}" name="testimoni" id="testimoni">{!! old('testimoni', $testimoni->testimoni) !!}</textarea>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Testimoni::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $testimoni->status) === (string) $key ? 'checked' : '' }}>
