    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.emergency-contacts.update", [$emergencyContact->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('candidate') ? 'is-invalid' : '' }}" name="candidate_id" id="candidate_id">
                    @foreach($candidates as $id => $entry)
                        <option value="{{ $id }}" {{ (old('candidate_id') ? old('candidate_id') : $emergencyContact->candidate->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('contact_number') ? 'is-invalid' : '' }}" type="text" name="contact_number" id="contact_number" value="{{ old('contact_number', $emergencyContact->contact_number) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('e_mail_address') ? 'is-invalid' : '' }}" type="text" name="e_mail_address" id="e_mail_address" value="{{ old('e_mail_address', $emergencyContact->e_mail_address) }}">
