...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.formalEducation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.formal-educations.update", [$formalEducation->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="school_academy">{{ trans('cruds.formalEducation.fields.school_academy') }}</label>
                <input class="form-control {{ $errors->has('school_academy') ? 'is-invalid' : '' }}" type="text" name="school_academy" id="school_academy" value="{{ old('school_academy', $formalEducation->school_academy) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('from_date') ? 'is-invalid' : '' }}" type="text" name="from_date" id="from_date" value="{{ old('from_date', $formalEducation->from_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('to_date') ? 'is-invalid' : '' }}" type="text" name="to_date" id="to_date" value="{{ old('to_date', $formalEducation->to_date) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('qualification_attained') ? 'is-invalid' : '' }}" type="text" name="qualification_attained" id="qualification_attained" value="{{ old('qualification_attained', $formalEducation->qualification_attained) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('candidate') ? 'is-invalid' : '' }}" name="candidate_id" id="candidate_id">
                    @foreach($candidates as $id => $entry)
                        <option value="{{ $id }}" {{ (old('candidate_id') ? old('candidate_id') : $formalEducation->candidate->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
