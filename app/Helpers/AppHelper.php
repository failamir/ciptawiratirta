    ...
    if(!empty($setting))
    {
        foreach($setting as $l=>$menuId){
            if($l == $location and $menuId){
                ...
                $walker = new $options['walker']($translation, $options);
...
function get_user_view($userId){
    $clientIp = request()->ip();
    $userViews = UserViews::where('user_id',$userId)->where('client_ip',$clientIp)->whereDate('created_at',Carbon::today())->count();
    if (!$userViews){
...
if (!function_exists('bc_obfuscate_email')) {
    function bc_obfuscate_email($email)
    {
        $em = explode("@", $email);
