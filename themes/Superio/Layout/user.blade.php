...
<head>
    ...
    <link href="{{ asset('themes/superio/dist/frontend/css/app.css?_ver=' . config('app.asset_version')) }}"
        rel="stylesheet">
    <!-- Bootstrap JS (required for tabs) -->
    <script src="{{ asset('themes/superio/assets/js/bootstrap.bundle.min.js') }}"></script>
...
<body data-anm=".anm"
    class="frontend-page user_wrap {{ $body_class ?? '' }} @if (!empty($is_home) or !empty($header_transparent)) header_transparent @endif @if (setting_item_with_lang('enable_rtl')) is-rtl @endif @if (is_api()) is_api @endif">
