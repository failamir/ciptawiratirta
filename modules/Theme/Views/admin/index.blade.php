                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                <h5 class="card-title">{{$themeClass::$name}}
                                    @if(\Modules\Theme\ThemeManager::current() == $theme_id)
                                        <span class="badge badge-warning">{{__("Current theme")}}</span>
                                    @endif
                                </h5>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    @if(\Modules\Theme\ThemeManager::current() == $theme_id)
                                        <form class="d-none" onsubmit="return confirm('{{__("Do you want to import all demo data?")}}')" action="{{route('theme.admin.seeding',['theme'=>$theme_id])}}" method="post">
                                            @csrf
