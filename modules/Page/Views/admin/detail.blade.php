                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        ...
                        @if(is_default_lang())
                            ...
                            @if(count($core['header_style']) > 1)
                                <div class="panel">
                                    <div class="panel-title"><strong>{{__('Header Style')}}</strong></div>
                                    <div class="panel-body">
                                        <select name="header_style" class="form-control" >
                                            @foreach($core['header_style'] as $key => $val)
                                                <option value="{{ $key }}" {{ ( $row->header_style ?? '') == $key ? 'selected' : ''  }}>{{ $val }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="panel-body">
                                        <select name="footer_style" class="form-control" >
                                            @foreach($core['footer_style'] as $key => $val)
                                                <option value="{{ $key }}" {{ ( $row->footer_style ?? '') == $key ? 'selected' : ''  }}>{{ $val }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            @php do_action(\Modules\Page\Hook::PAGE_SETTING_AFTER_FEATURE_IMAGE, $row); @endphp
                        @endif
                    </div>
                </div>
            </div>
