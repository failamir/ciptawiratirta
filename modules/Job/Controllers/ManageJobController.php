    ...
    public function storeJob(Request $request, $id){
        ...
        if($row->status == 'publish' and !auth()->user()->checkJobPlan()){
            if($input['status'] == 'publish'){
                if(!setting_item('job_require_plan')){
                    return back()->withInput()->with('error',__("Maximum published items reached. You can save drafts or buy more plan. ") . '<a href="'. route('plan') .'" class="decoration-underline">'.__("Buy now") . '</a>');
                }else{
                    return back()->withInput()->with('error',__("Maximum published items reached. You can save drafts."));
                }
            }else{
                $row->status = 'draft';
            }
