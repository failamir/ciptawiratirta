...
<div class="form-group row ">
    ...
    <div class="col-md-3">
        <select @if(!is_default_lang()) readonly @endif name="cat_id" required class="form-control">
            ...
            <?php
            $items = \Modules\Gig\Models\GigCategory::query()->where("status", "publish")->whereNull('parent_id')->get();
            ?>
    ...
    <div class="col-md-3">
        <select @if(!is_default_lang()) readonly @endif name="cat2_id" required class="form-control">
            ...
            <?php
            $items = \Modules\Gig\Models\GigCategory::query()->where("status", "publish")->withDepth()->having('depth', '=', 1)->get();
            ?>
            @foreach($items as $item)
    ...
    <div class="col-md-3">
        <select @if(!is_default_lang()) readonly @endif name="cat3_id" required class="form-control">
            ...
            <?php
            $items = \Modules\Gig\Models\GigCategory::query()->where("status", "publish")->withDepth()->having('depth', '=', 2)->get();
            ?>
            @foreach($items as $item)
